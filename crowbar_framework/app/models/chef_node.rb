...
class ChefNode < ChefObject
  include Crowbar::ConduitResolver

  self.chef_type = "node"

  def initialize(node)
    @role = RoleObject.find_role_by_name ChefNode.make_role_name(node.name)
    if @role.nil?
      # An admin node can exist without a role - so create one
      if !node["crowbar"].nil? and node["crowbar"]["admin_node"]
        @role = ChefNode.create_new_role(node.name, node)
      else
  ...
  def validate_alias(value, unique_check = true)
    ...
    if unique_check
      node = ChefNode.find_node_by_alias value
  ...
  def validate_public_name(value, unique_check = true)
    unless value.to_s.empty?
      ...
      if unique_check
        node = ChefNode.find_node_by_public_name value
  ...
  class << self
    ...
    def default_platform
      @default_platform ||= begin
        provisioner = ChefNode.find("roles:provisioner-server").first
    ...
    def create_new(new_name)
      ...
      role = RoleObject.find_role_by_name ChefNode.make_role_name(new_name)
      role = ChefNode.create_new_role(new_name, machine) if role.nil?
      ChefNode.new machine
    end
  end
end
