  ...
  describe "finders" do
    ...
    describe "find_node_by_name_or_alias", find_node_by_name_or_alias: true do
      ...
      it "returns nodes matching alias (second part of method)" do
        testing_node = Node.find_node_by_name("testing.crowbar.com")
        allow(Node).to receive(:find_node_by_alias).with("testing_alias").and_return(testing_node)
        node = Node.find_node_by_alias("testing_alias")
        expect(node).to_not be_nil
      end
    end
  end
